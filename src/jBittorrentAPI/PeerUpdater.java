...
public class PeerUpdater extends Thread {
    ...
    public void run() {
        ...
        while (!this.end) {
            ...
            for(String announce : torrent.announceURLs) {
            	LinkedHashMap<String, Peer> hash = this.processResponse(this.contactTracker(
	            		announce, id,
	                    torrent, this.downloaded,
	                    this.uploaded,
	                    this.left, this.event));
            	if (hash != null)
            		this.peerList.putAll(hash);
            }
    ...
    public synchronized Map contactTracker(
    									   String announce, byte[] id,
                                           TorrentFile t, long dl, long ul,
                                           long left, String event) {
    	
        try {
            URL source = new URL(announce + (announce.endsWith("&") ? "" : "?") + "info_hash=" +
...
//            System.out.println(m);
    ...
    public void end() {
        ...
        for(String announce : this.torrent.announceURLs)
        	this.contactTracker(announce ,this.id, this.torrent, this.downloaded,
