...
public class TorrentProcessor {
     ...
     * @return TorrentFile
     */
    public TorrentFile getTorrentFile(Map m, String saveTo){
    	this.torrent.saveTo = saveTo;
        if(m == null)
            return null;
        if(m.containsKey("announce")) // mandatory key
            this.torrent.announceURLs.add(new String((byte[]) m.get("announce")));
        if (m.containsKey("announce-list")) {
        	List<ArrayList> announces = (List<ArrayList>) m.get("announce-list");
            for (ArrayList announce : announces)
            	this.torrent.announceURLs.add(new String((byte[])announce.get(0)));
        }
        
        if (this.torrent.announceURLs.size() == 0)
        	return null;
    ...
    public void setTorrentData(String url, int pLength, String comment,
    	...
    	this.torrent.announceURLs.add(url);
    ...
    public byte[] generateTorrent(TorrentFile torr) {
        ...
        map.put("announce", torr.announceURLs.get(0));
        
        map.put("announce-list", torr.announceURLs);
